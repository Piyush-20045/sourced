...
import { clientSideNav } from "@/data/dashboard/client-dashboard";
...
/** Left-hand section navigation for the client dashboard area. */
export function ClientSideNav({ activeTab, onSelectTab }: ClientSideNavProps) {
  const [internalActive, setInternalActive] = useState(clientSideNav[0]!.label);
  ...
  return (
    <nav className="space-y-1 text-sm">
      {clientSideNav.map((item, i) => {
        const Icon = icons[item.icon as keyof typeof icons] || LayoutGrid;
        const isNewSection =
          item.section && item.section !== clientSideNav[i - 1]?.section;
